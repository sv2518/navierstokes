...
def plot_convergence_velo_pres(error_velo,error_pres,list_N,value,label):
    # velocity convergence plot
    fig = plt.figure(1)
    axis = fig.gca()
    linear=error_velo
    axis.loglog(list_N,linear,label=label)
    axis.loglog(list_N,value*np.power(list_N[::1],2),'r*',label="second order")
    axis.loglog(list_N,value*np.power(list_N[::1],1),'g*',label="first order")
    axis.set_xlabel('$dx$')
