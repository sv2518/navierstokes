...
def plot_convergence_velo_pres(error_velo,error_pres,list_N,value):
    ...
    axis = fig.gca()
    linear=error_velo
    axis.semilogy(list_N,linear,label='$||e_u||_{\infty}$')
    axis.semilogy(list_N,value*np.power(list_N,2),'r*',label="second order")
    axis.semilogy(list_N,value*np.power(list_N,1),'g*',label="first order")
