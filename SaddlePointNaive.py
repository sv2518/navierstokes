...
def initialPressure(W,U,P,mesh,nue,bc,u_init,U_inf,parameters_velo,parameters_pres,dt,bc_tang):
    ...
    un_pos = 0.5*(dot(u_linear,n)*u+abs(dot(u_linear,n))*u+ dot(u_linear,n)*bc_tang-abs(dot(u_linear,n))*bc_tang) 
    adv_dg=(
        dot(u,div(outer(v,u_linear)))*dx#like paper
            -dot(v,un_pos)*ds(4)#similar to matt piggots
...
def buildPredictorForm(p_init_sol,u_init_sol,nue,mesh,U,P,W,U_inf,dt,bc_tang,v_k,u_n,p_n):
    ...
    un_pos = 0.5*(dot(ubar_k,n)*ubar_knew+abs(dot(ubar_k,n))*ubar_knew + dot(ubar_k,n)*bc_tang-abs(dot(ubar_k,n))*bc_tang)
    #for lid
...
def solve_problem(mesh_size,parameters_corr, parameters_pres,parameters_velo_initial, parameters_velo,parameters_4, aP=None, block_matrix=False):
    ...
    #function spaces
    U = FunctionSpace(mesh, "RTCF",3)
    P = FunctionSpace(mesh, "DG", 2)
    W = U*P
